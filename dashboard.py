...
class ResultManagementSystem:
    def __init__(self,home):
        ...
        #SubMenu
        button_Course=Button(Frame,text="Course",font=("ties new roman",15,"bold"),bg="#4477CE",fg="white",cursor="hand2",command=self.add_course).place(x=20,y=5,width=200,height=40)

        button_Student=Button(Frame,text="Student",font=("ties new roman",15,"bold"),bg="#4477CE",fg="white",cursor="hand2",command=self.add_student).place(x=240,y=5,width=200,height=40)

        button_Result=Button(Frame,text="Result",font=("ties new roman",15,"bold"),bg="#4477CE",fg="white",cursor="hand2",command=self.add_result).place(x=460,y=5,width=200,height=40)

        button_View=Button(Frame,text="View Student Result",font=("ties new roman",15,"bold"),bg="#4477CE",fg="white",cursor="hand2",command=self.add_view).place(x=680,y=5,width=205,height=40)

        button_Logout=Button(Frame,text="Logout",font=("ties new roman",15,"bold"),bg="#4477CE",fg="white",cursor="hand2",command=self.logout).place(x=900,y=5,width=200,height=40)

        button_Exit=Button(Frame,text="Exit",font=("ties new roman",15,"bold"),bg="#4477CE",fg="white",cursor="hand2",command=self.exit).place(x=1120,y=5,width=200,height=40)

        # Content Window
        self.bgImage=Image.open("Images/motivation-creates-your-future.jpg")
        self.bgImage=self.bgImage.resize((935,350),Image.Resampling.LANCZOS)
        self.bgImage=ImageTk.PhotoImage(self.bgImage)

        self.lbl_bg=Label(self.home,image=self.bgImage).place(x=220,y=180,width=940,height=350)

        # Update Details
        self.totalCourse=Label(self.home,text="Total Courses \n 0 ",font=("times new roman",20),bd=10,relief=RIDGE,bg="purple",fg="white")
        self.totalCourse.place(x=220,y=530,width=300,height=80)
        self.totalstudent=Label(self.home,text="Total Student \n 0 ",font=("times new roman",20),bd=10,relief=RIDGE,bg="orange",fg="white")
        self.totalstudent.place(x=542,y=530,width=300,height=80)

        self.totalresults=Label(self.home,text="Total Results \n 0 ",font=("times new roman",20),bd=10,relief=RIDGE,bg="coral",fg="white")
        self.totalresults.place(x=860,y=530,width=300,height=80)
